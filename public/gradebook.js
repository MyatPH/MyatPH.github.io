function fetchGradeData() {
  ...
  xhr.onreadystatechange = function() { // <--- MAKE SURE THERE IS AN "=" HERE
    let results;
    // Check if we're done
    if (xhr.readyState === xhr.DONE) {
      // Check if we're successful
      if (xhr.status !== 200) {
        console.error(`Could not get grades. Status: ${xhr.status}`); // And confirm backticks are used here
      }
...
let tableBody = tableElm.querySelector("tbody"); // Get the gradebook table element
...
// Call fetchGradeData when the DOM is fully loaded
